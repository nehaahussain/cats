...
def pick(paragraphs, select, k):
    ...
    count = 0
    for i in paragraphs:
        if select(i) == True and count == k:
            count += 1
            return i 
        elif select(i) == True and count != k: 
            count += 1   
    return ''
...
def accuracy(typed, source):
    ...
    while k < min(len(source_words), len(typed_words)):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in word_list[1:]:
        if diff_function(typed_word, i, limit) < result:
            result = diff_function(typed_word, i, limit)
            word = i
    if result > limit:
        return typed_word
    return word 
...
def fastest_words(match):
    ...
    fastest_players = []
    lowest_time = 0
    player_w_lowest_time = 0 #for index i in this list, this signifies the player who typed word i the fastest
    for i in word_indices:
        lowest_time = time(match, 0, i) 
        player_w_lowest_time = 0
        for d in player_indices: 
            if time(match, d, i) < lowest_time :
                lowest_time = time(match, d, i) 
                player_w_lowest_time = d
        fastest_players.append(player_w_lowest_time)
    for i in range(len(get_all_times(match))): 
        output += [[]] 
    for i in range(len(fastest_players)): 
        output[fastest_players[i]] += [get_word(match, i)]
    return output 
